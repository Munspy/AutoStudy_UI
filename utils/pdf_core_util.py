...
이 모듈은 PDF 파일의 물리적 조작(병합, 분할, 재조합) 및 UI 렌더링을 위한 이미지 변환 기능을 제공합니다.
전체 애플리케이션 파이프라인에서 파일 I/O 및 `pymupdf` 라이브러리와 직접 맞닿아 있는 로우레벨(low-level) 
코어 계층을 담당하며, 상위 UI 레이어나 비즈니스 로직에서 PDF 내부 구조를 몰라도 손쉽게 다룰 수 있도록 
추상화된 인터페이스를 제공합니다. 자원 반환(메모리 누수 방지)을 위한 컨텍스트 매니저 사용 및 
파일 저장 시 용량 최적화 로직이 일관되게 적용되어 있습니다.
"""
...
def get_page_image_bytes(
    ...
    호출하는 측에서 자유롭게 바이트 데이터를 활용해 이미지를 렌더링할 수 있습니다.
    ...
    Args:
        ...
        zoom (float, optional): 렌더링 해상도 배율입니다. 기본값은 1.0이며, 값이 커질수록 고해상도 이미지가 생성됩니다.
        image_format (str, optional): 출력할 이미지의 포맷을 지정합니다 (예: "png", "jpeg"). 기본값은 "png"입니다.

    Returns:
        bytes | None: 성공적으로 렌더링된 이미지의 순수 바이트 데이터입니다.
                      페이지 번호가 범위를 벗어나거나, 파일을 찾을 수 없거나, 손상된 경우 None을 반환합니다.
    """
    try:
...
def merge_pdfs(pdf_paths: List[PathLike], output_path: PathLike) -> str:
    ...
    사용자가 여러 개의 쪼개진 문서를 하나로 취합할 때 사용되는 핵심 병합 로직입니다.
    파일을 단순히 이어붙이는 것에 그치지 않고, `pymupdf.open()`의 컨텍스트 매니저를 통해 자원 누수를 원천 차단합니다.
    특히 PDF 병합 시 발생할 수 있는 내부 객체 파편화 및 파일 용량 팽창 문제를 해결하기 위해,
    최종 저장 시 `garbage=4`(미사용 객체 및 중복 데이터 완전 정리)와 `deflate=True`(스트림 압축) 옵션을 강제합니다.
    중간에 존재하지 않거나 손상된 파일이 포함되어 있더라도 오류를 발생시키지 않고 해당 파일만 스킵하여 안정성을 높였습니다.

    Args:
        pdf_paths (List[PathLike]): 병합할 대상 PDF 파일 경로들의 리스트입니다. 리스트의 순서대로 페이지가 병합됩니다.
        output_path (PathLike): 병합이 완료된 결과물 PDF를 저장할 최종 경로입니다. 폴더가 없다면 자동 생성됩니다.

    Returns:
        str: 정상적으로 병합되어 저장된 최종 결과물 파일의 절대/상대 경로 문자열입니다.
...
def split_pdf_two_parts(
    ...
    페이지 범위를 복사하는 방식을 채택하였으며, 저장 시에는 병합 로직과 동일하게 최적화 및 압축 옵션을 
    적용하여 용량 효율성을 확보합니다.
...
def merge_specific_pages(
    ...
    여러 문서에서 필요한 페이지들만 골라내어(예: A문서 3쪽, B문서 1쪽, A문서 5쪽) 새로운 문서를 조립하는
    복잡한 재구성(re-ordering/extracting) 시나리오에 대응하는 핵심 로직입니다. 
    사용자가 동일한 파일을 여러 번 번갈아가며 요청할 수 있으므로, 동일 파일을 열고 닫기를 반복하면 
    심각한 I/O 오버헤드가 발생합니다. 이를 방지하고자 내부 딕셔너리(`opened_pdfs`)를 이용한 파일 핸들 
    캐싱(메모이제이션) 기법을 사용하며, 가변적인 개수의 파일 핸들을 예외 발생 시에도 안전하게 한꺼번에 
    닫아주기 위해 `contextlib.ExitStack`을 활용한 고도화된 컨텍스트 관리를 수행합니다.
    ...
    Returns:
        str: 맞춤 병합 및 최적화 저장이 완료되어 생성된 최종 PDF 파일의 경로 문자열입니다.

    Raises:
        ValueError: `page_recipe` 리스트가 비어 있어 병합할 페이지 정보가 입력되지 않은 경우 발생합니다.
    """
    if not page_recipe:
