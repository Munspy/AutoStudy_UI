...
def _get_yt_service():
    ...
    return AppContainer.get_instance().yt_playlist

def load_csv_data(): return _repo.load_playlists()
def parse_playlist_id(url): return _get_yt_service().parse_playlist_id(url)
...
def get_playlist_title(url): return _get_yt_service().get_playlist_title(url)
...
class YoutubePlaylistController(BaseController):
    ...
    def start_fetch_playlist(self, playlist_id: str):
        ...
        # 조회 완료 시그널 연결
        worker.finished_signal.connect(self.fetch_completed.emit)
        # 백그라운드 워커 시작
    ...
    def start_update_checker(self, playlists: list):
        ...
        # 확인 작업 완료 시그널 연결
        worker.finished_signal.connect(self.checker_completed.emit)
        # 백그라운드 워커 시작
        self.start_worker(worker)
