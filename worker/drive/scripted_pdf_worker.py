...
class ScriptedPdfMergeWorker(BaseWorker):
    ...
    def do_work(self):
        ...
        self._log(f"🚀 총 {len(self.checked_lessons)}개 선택된 수업의 스크립트 합본 다운로드 및 병합 작업을 시작합니다...")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for idx, lesson_id in enumerate(sorted_lessons, 1):
                ...
                if not file_info:
                    self._log(f"⚠️ [{lesson_id}] _scripted.pdf 파일이 구글 드라이브에 존재하지 않아 건너뜁니다.")
                    continue

                local_file = temp_path / f"{lesson_id}_scripted.pdf"
                self._log(f"📥 [{idx}/{total}] [{lesson_id}] _scripted.pdf 다운로드 중...")
                self.progress_signal.emit(int((idx / total) * 80), f"[{lesson_id}] 다운로드 중...")
                
                try:
                    self.app.drive_client.download_from_drive(file_info['id'], str(local_file))
                    # timetable 기반 목차(TOC) 제목 설정
                    tt_info = self.app.timetable.find_timetable_info_for_lesson(lesson_id)
                    ...
                    self._log(f"❌ [{lesson_id}] 다운로드 실패: {str(e)}")
            ...
            # 4. 목차가 포함된 PDF 병합 생성
            self._log(f"🔗 {len(pdf_entries)}개 수업 PDF를 목차(TOC)와 함께 강의 순서대로 병합하는 중...")
            self.progress_signal.emit(90, "PDF 병합 및 목차 생성 중...")
            
            success, msg = self.app.pdf_operation.merge_scripted_pdfs_and_save(pdf_entries=pdf_entries, output_path=self.output_path, cancel_checker=self.is_cancelled)
            if not success:
                self.error_signal.emit(msg)
                return None

            self.progress_signal.emit(100, "완료")
            self._log(f"🎉 스크립트 합본 다운로드 및 병합 완료! (저장 위치: {self.output_path})")
            return msg
